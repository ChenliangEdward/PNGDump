...
def extend(circuit, node_router):
    ...
    # secret_input = shared_X__y + shared_X__b + node_ID + public_B + public_X + public_Y +
    # b"ntor-curve25519-sha256-1"  # your-code-here#
...
def get(hostname, port, path="", guard_address=None, middle_address=None, exit_address=None):
    ...
    if guard_address:
        guard_route = router_from_ip(guard_address, consensus)
    else:
        guard_route = random_router(all_relays)
    if middle_address:
        middle_route = router_from_ip(middle_address, consensus)
    else:
        middle_route = random_router(all_relays)
    if exit_address:
        exit_route = router_from_ip(exit_address, consensus)
    else:
        exit_route = random_router(get_all_exits(consensus))

    circuit_base = gen_circuit_id()  # your-code-here#  # CREATE
    circuit = circuit_from_guard(guard_route, circuit_base)

    # your-code-here#  # EXTEND
    circuit = circuit_build_hops(circuit, middle_route, exit_route)
    ...
    template = b'GET /%s HTTP/1.0\r\nHost: %s\r\n\r\n'
    request = template % (path.encode(), hostname.encode())
